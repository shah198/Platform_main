...
export class Tenant_AppsRoutes{
        ...
        return async (result) => {
          ...
          try {
            ...
            for (let index = 0; index < result.OnFailureTopicsToPush.length; index++) {
              ...
              let requestModelOfTenant_AppsDto: RequestModel<Tenant_AppsDto> = result["message"];
              errorResult.setSocketId(requestModelOfTenant_AppsDto.SocketId)
              errorResult.setCommunityUrl(requestModelOfTenant_AppsDto.CommunityUrl);
              errorResult.setRequestId(requestModelOfTenant_AppsDto.RequestGuid);
  ...
  @Get(':id')
    TenantAppById(@Param('id') id: number): Promise<ResponseModel<Tenant_AppsDto>> {
    try {
      console.log("id is............." + JSON.stringify(id));
      return this.tenant_appsFacade.getByIds([id]);
    } catch (error) {
      throw new HttpException(error, HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
